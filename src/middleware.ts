...
export function middleware(req: NextRequest) {
  ...
  const lngInPath = languages.find(loc => loc !== fallbackLng && req.nextUrl.pathname.startsWith(`/${loc}`));
  const headers = new Headers(req.headers);
  headers.set(headerName, lngInPath || lng);

  // If the language is not in the path and not the fallback language, redirect to include it
  if (!lngInPath && lng !== fallbackLng && !req.nextUrl.pathname.startsWith('/_next')) {
    return NextResponse.redirect(new URL(`/${lng}${req.nextUrl.pathname}${req.nextUrl.search}`, req.url));
  }

  //if the language is the fallback language and is in the path, redirect to remove it from the path
